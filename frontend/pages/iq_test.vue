<template>
    <div>
      ...
      <div class="b-color footer">
        ...
        <span class="ans-count1 text-dark font-weight-bolder" v-show="!finish && !oneleft"
          >{{choiceAns.left}} questions are still left. </span
        >
        <span class="ans-count1 text-dark font-weight-bolder" v-show="oneleft"
          >{{choiceAns.left}} question is still left. </span
        >

        <span class="ans-count1 text-dark font-weight-bolder"
        v-show="finish"  > All Done! </span
        >
       ...
       <!--Next Session-->
        <div v-if="nextRoute">
          <button class="btn btn-light next-btn">
            ...
            <NuxtLink :to="nextRoute" class="l-0 next-route text-white font-weight-bold">➡️</NuxtLink>
          </button>
        </div>
        <div v-else id="btn-next" v-show="finish">
          <button class="btn btn-success font-weight-bolder text-center mr-5" @click="submitAns()">Submit</button>
        </div>
      </div>
    </div>
  </template>
